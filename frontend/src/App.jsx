...
function Layout({ children, session, logout }) {
  ...
  const items = session?.role === "officer" ? [
    ["/officer", "▦", "Officer Dashboard"], ["/complaints", "＋", "Complaints"], ["/ward-analytics", "▥", "Ward Analytics"], ["/hotspots", "⌖", "Hotspots"], ["/relationships", "⌘", "Relationships"], ["/priority", "↑", "Priority Ranking"], ["/notices", "▣", "Notices"], ["/about", "ⓘ", "About"], ["/help", "?", "Help Desk"]
  ] : [["/citizen", "⌂", "Citizen Home"], ["/complaints", "＋", "Report Complaint"], ["/my-complaints", "✓", "My Complaints"], ["/hotspots", "⌖", "City Hotspots"], ["/notices", "▣", "Notices"], ["/predictor", "◉", "Risk Predictor"], ["/about", "ⓘ", "About"], ["/help", "?", "Help Desk"]];
...
function PriorityRanking(){return <><PageHeader eyebrow="DECISION SUPPORT" title="Priority Ranking" description="Ranked Jabalpur hotspots by severity, persistence and impact."/><section className="panel ranking-panel"><div className="ranking-list">{[...hotspotDisplay].sort((a,b)=>b.score-a.score).map((h,i)=><div className="ranking-row" key={h.id}><div className="ranking-number">{String(i+1).padStart(2,"0")}</div><div className="ranking-location"><strong>{h.title}</strong><span>{h.location}</span></div><div className="ranking-metric"><span>Complaints</span><strong>{h.complaints}</strong></div><div className="ranking-metric"><span>Persistence</span><strong>{h.persistence}%</strong></div><div className="ranking-score"><strong>{h.score}</strong><SeverityBadge severity={h.severity}/></div></div>)}</div></section></>}
...
function HelpDesk(){return <><PageHeader eyebrow="CITIZEN SUPPORT" title="Help Desk" description="Quick answers and support for reporting civic problems through UrbanNova."/>
...
function About(){return <><PageHeader eyebrow="ABOUT URBANNOVA" title="Jabalpur Civic Administration" description="UrbanNova is a civic intelligence interface for citizens and municipal teams."/>
...
function Predictor(){return <><PageHeader eyebrow="PREDICTIVE CIVIC INTELLIGENCE" title="Future Issue Predictor" description="Demo predictions based on recurring complaint patterns and known problem relationships. These are advisory, not guaranteed forecasts."/><div className="prediction-grid">{predictions.map(p=><div className="panel prediction-card" key={p.id}><div className="prediction-top"><span className={`risk-pill ${p.risk.toLowerCase()}`}>{p.risk} Risk</span><span>{p.window}</span></div><h2>{p.issue}</h2><strong>{p.area}</strong><p>{p.reason}</p><div className="prediction-action"><b>Recommended preventive action</b><span>{p.action}</span></div></div>)}</div></>}
